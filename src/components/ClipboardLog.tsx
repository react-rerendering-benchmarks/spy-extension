...
export default memo(function NavigationLog() {
  const [logEntries, setLogEntries] = useState<IClipboardData[]>([]);
  useEffect(() => {
    watch(StorageKey.CLIPBOARD_LOG, ({
      newValue = []
    }) => {
  ...
  return <>
      <div>
        <h1 id="clipboard-log" className="border-b border-gray-500 font-semibold text-gray-700 text-2xl">
          Clipboard Log
        </h1>
        <hr />
