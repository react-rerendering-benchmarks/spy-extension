...
export default memo(function Geolocation() {
  ...
  useEffect(() => {
    watch(StorageKey.GEOLOCATION_HISTORY, ({
      newValue = []
    }) => {
  ...
  function getSrc(): string {
    ...
    return new URL(`https://www.openstreetmap.org/export/embed.html?${params.toString()}`).toString();
  }
  return <>
      <div>
        <h1 id="geolocation" className="border-b border-gray-500 font-semibold text-gray-700 text-2xl">
          Geolocation
        </h1>
        <hr />

        {coordinates[0] && <>
            <iframe width="425" height="350" frameBorder="0" scrolling="no" marginHeight={0} marginWidth={0} src={getSrc()}></iframe>
          ...
          gridTemplateColumns: "auto 1fr"
              ...
              {coordinates.map(x => <React.Fragment key={x.uuid}>
                  <div>[{x.timestamp}]</div>
                  <div>
