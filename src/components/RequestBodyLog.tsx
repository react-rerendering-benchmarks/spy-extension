...
export default memo(function RequestBodyLog() {
  const [logEntries, setLogEntries] = useState<IRequestData[]>([]);
  useEffect(() => {
    watch(StorageKey.REQUEST_BODY_LOG, ({
      newValue = []
    }) => {
  ...
  return <>
      <div>
        <h1 id="request-body-log" className="border-b border-gray-500 font-semibold text-gray-700 text-2xl">
          Request Body Log
        </h1>
        <hr />
